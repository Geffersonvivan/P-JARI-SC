#!/bin/bash
# Script de inicialização do Celery Beat — P-JARI
# Chamado pelo launchd (~/Library/LaunchAgents/com.pjari.celery-beat.plist)
...
exec "$PYTHON" -m celery -A config beat \
    --loglevel=info \
    --logfile="$PROJECT_DIR/logs/celery-beat.log" \
    --pidfile="$PROJECT_DIR/logs/celery-beat.pid"
