...
class GeminiClient:
    ...
    def generate_phase2_report(self, parecer_obj, contexto_textual_datas):
        ...
        system_instruction = (
            ...
            "A saída deve começar obrigatoriamente com as seguintes linhas (nessa ordem):\n"
            "RECORRENTE: [NOME DO CONDUTOR OU PROPRIETÁRIO IDENTIFICADO NO PDF]\n"
            "TIPO_PENALIDADE: [multa | advertencia | suspensao | cassacao] — tipo da autuação conforme documento\n"
            "DATA_CONCLUSAO_MULTA: [DD/MM/AAAA ou NÃO SE APLICA] — data de conclusão do processo de multa que originou a suspensão/cassação (preencher apenas se TIPO_PENALIDADE for suspensao ou cassacao e a data estiver no documento; caso contrário, escrever NÃO SE APLICA)\n"
            "TEM_FLAGRANTE: [SIM | NAO | NAO_DETERMINADO] — a autuação foi lavrada em flagrante? SIM = agente de trânsito presenciou o ato; NAO = autuação eletrônica/radar/câmera sem presença do agente; NAO_DETERMINADO = não é possível determinar pelo documento\n"
            "DATA_CONHECIMENTO_INFRACAO: [DD/MM/AAAA ou NÃO SE APLICA] — data em que o órgão tomou ciência da infração (art. 282 §6º-A CTB); preencher apenas se TEM_FLAGRANTE for NAO e a data constar no documento; caso contrário, escrever NÃO SE APLICA\n\n"
        ...
        if parecer_obj.ata_pdf_path and "upload_simulado" not in _p(parecer_obj.ata_pdf_path):
            try:
                if default_storage.exists(_p(parecer_obj.ata_pdf_path)):
                    file_ata = self.upload_file(_p(parecer_obj.ata_pdf_path))
                    if file_ata:
    ...
    def generate_phase3_report(self, parecer_obj, matematica_detalhes):
        ...
        system_instruction = (
            ...
            "Exemplo: se Prescrição Punitiva: NÃO → escreva [DECISAO_ADMISSIBILIDADE_PUNITIVA:NAO].\n\n"
    ...
    def analyze_tese(self, parecer_obj, tese, perplexity_result, vertex_result):
        ...
        system_instruction = (
            ...
            "   - Pule uma linha (\\n\\n) e escreva: '**Tese X – Alternativa (a) – Acolhimento:**' e explique em texto corrido e conciso (até 4 linhas) a linha de raciocínio para ACOLHIMENTO da tese, à luz do normativo (RAG), precedentes e prova dos autos. Indicar principais dispositivos/resoluções de forma objetiva. Deixar claro as premissas fáticas necessárias para sustentar essa alternativa.\n"
            "   - OBRIGATÓRIO: Pule outra linha dupla (\\n\\n) antes de iniciar a alternativa (b).\n"
            "   - Escreva: '**Tese X – Alternativa (b) – Não acolhimento:**' e explique em texto corrido e conciso (até 4 linhas) a linha de raciocínio para NÃO ACOLHIMENTO, considerando a presunção de legitimidade, ausência de prova em contrário (se for o caso) e parâmetros do RAG. Indicar principais dispositivos legais aplicáveis. Deixar claro se a rejeição é por: (i) ausência de prova, (ii) irrelevância jurídica, ou (iii) incompatibilidade com entendimento consolidado.\n"
...
class AnthropicClient:
    ...
    def validate_and_generate_parecer(self, parecer_obj, tese, perplexity_result, vertex_result="", task_id=None):
        ...
        if parecer_obj.autuacao_pdf_path and "upload_simulado" not in _p(parecer_obj.autuacao_pdf_path):
            pdf_data = self.get_pdf_content(_p(parecer_obj.autuacao_pdf_path))
            if pdf_data: content.append(pdf_data)
            
        if parecer_obj.consolidado_pdf_path and "upload_simulado" not in _p(parecer_obj.consolidado_pdf_path) and _p(parecer_obj.consolidado_pdf_path) != _p(parecer_obj.autuacao_pdf_path):
            pdf_data = self.get_pdf_content(_p(parecer_obj.consolidado_pdf_path))
            if pdf_data: content.append(pdf_data)
